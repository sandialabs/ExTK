...
The `ExplodeContract` script takes the selected model and create a list of original model positions and exploded model positions.
  ...
  - `ExplodeAndContract(Vector3,Vector3,int)`: This method takes in three parameters a [`Vector3`](https://docs.unity3d.com/ScriptReference/Vector3.html) of the current model position, a [`Vector3`](https://docs.unity3d.com/ScriptReference/Vector3.html) of the move position to create the distance the model will move, and an `integer` for the total number of submodels that need to be moved. The method is used to create the move distance for each sub-model item.
...
The `Move` script moves the selected `gameObject` **Up**, **Down**, **Left**, **Right**, **Forward**, or **Back**.
   ...
   - `Movement(Vector3)`: This method changes the [`Vector3`](https://docs.unity3d.com/ScriptReference/Vector3.html) position of the model based on the [`Vector3`](https://docs.unity3d.com/ScriptReference/Vector3.html) passed in.
